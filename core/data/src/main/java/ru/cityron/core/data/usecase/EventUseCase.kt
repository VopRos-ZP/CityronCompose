...
import ru.cityron.core.domain.utils.Temp
import ru.cityron.core.domain.utils.Time
...
class EventUseCase @Inject constructor(
    ...
    fun fetchEvents(
        ...
        sources: Int,
        reasons: Int
    ): Flow<Resource<List<EventWithDate>>> = flow {
        emit(Resource.Loading(true))

        val events = eventsRepository.fetchEvents(count, types, sources, reasons).events

        val zone = events.zone
        val data = events.data

        val result = data.map {
            Event(
                date = Time.secondsToString(it[0], zone, Time.formatDatetime),
                type = when (it[1]) {
                    0 -> "АВАРИЯ"
                    1 -> "Конфиг"
                    2 -> "Сервис"
            ...
            it.copy(date = it.date.split(" ")[1])
        }.map { (k, v) -> EventWithDate(k, v) }
